...
source: https://docs.emergingtravel.com/docs/affiliate-api/booking/receive-booking-status-webhook/
tags:
- etg-docs
- affiliate-api
- booking
...
3. Use your API Key token as a key.
4. Use the SHA256 digest mode.
5. Use the `hexdigest()` method to make a resulting string.
6. Compare the resulting string to the signature.
7. **Optional.** Cache the token locally and don’t honor any subsequent request with the same token. This will prevent replay attacks.
8. **Optional.** Check that the timestamp is within the token lifetime.
